...
class Spectrum:    
    ...
    def fit_gaussian(self, lambdaT, plot=False, unique=False):
        velocities = []
        uncertainties = []
        avg_balmer=0
        avg_err= 0
        for lT in lambdaT:
            ...
            if len(velocities) == 1:#only enter this if block after two balmer lines
                avg_balmer = self.calculate_avg_velocity(velocities, uncertainties)
                avg_err = self.calculate_avg_uncertainty(uncertainties)
            ...
            velocities.append(v)
            uncertainties.append(err_velocity)
            

        avg_velocity = self.calculate_avg_velocity(velocities, uncertainties) 
        sigma_avg = self.calculate_avg_uncertainty(uncertainties)
    ...
    def calculate_avg_velocity(self, velocities, uncertainties):
        ...
        if b == 0:
        	print("Error: there are not enough values to calculate average velocity - possibly because of noise in spectrum")
            return ''
        else:  
        	return a/b
    ...
    def calculate_avg_uncertainty(self, uncertainties):
        ...
        if d == 0:
        	print("Error: there are not enough values to calculate average uncertainty - possibly because of noise in spectrum")
        	return ''
        else:	
        	return 1/sqrt(d)
    ...
    def show_walkers(self, chain, labels, wavelength, savefile=None):
    ...
    def show_corner_plot(self, samples,labels,truths, wavelength, savefile=None):
    ...
    def fit_mcmc(self, lambda_rest_arr, saveFile=False):
        min_vel=-100
        max_vel=100
        min_dv=0
        max_dv=0
        ...
        for i in range(len(lambda_rest_arr)):
            ...
            #print(str(lambda_rest)+": "+str(sigma))    
            ...
            sigmas.append(dv)
            if saveFile != False:
                fig, axes = self.show_walkers(sampler_gaussian.chain, ['amp', 'sigma', 'velocity', 'c'],lambda_rest,saveFile)
                fig = self.show_corner_plot(samples_gaussian, ['amp','sigma','velocity','c'], p_initial,lambda_rest,saveFile)
